...
> Boss **art assets** — the hermit-crab animation clips, the spawn-in dissolve shader, and the
> graybox→model swap — moved to `art-todo.md`. The code hooks that play/consume them stay here.
...
- [ ] Boss spawn-in **dissolve/materialize shader** (option B) — moved to `art-todo.md` (needs a material
      asset). The C++ hook to drive `Dissolve`/`EdgeColor` lands once the material exists.
...
> **Niagara VFX upgrade (art pass)** — authoring the Niagara systems (impact bursts, dust, death poof,
> loot beams, boss spawn-in, mage fireball `NS_Fireball`/`NS_FireballImpact`, etc.) moved to `art-todo.md`.
> The C++ spawn points already exist and auto-load by path once the systems are authored.
...
> **Art asset creation** (models, meshes, textures, icons, materials, animations, Niagara systems) now
> lives in `art-todo.md`.
