...
void ADungeonGenerator::SpawnRoomLights()
{
	UWorld* World = GetWorld();
	if (!World)
	{
	...
	for (const FDungeonRoom& Room : Rooms)
	{
		FVector Loc = GetActorTransform().TransformPosition(CellToLocal(Room.CenterX(), Room.CenterY()));
		Loc.Z += WallHeight * 0.75f;

		APointLight* Light = World->SpawnActor<APointLight>(
			APointLight::StaticClass(), FTransform(Loc), Params);
		if (Light)
		{
			if (UPointLightComponent* LC = Cast<UPointLightComponent>(Light->GetLightComponent()))
			{
				LC->SetMobility(EComponentMobility::Movable);
				LC->SetIntensity(RoomLightIntensity);
				LC->SetAttenuationRadius(FMath::Max(Room.W, Room.H) * CellSize * 1.5f);
